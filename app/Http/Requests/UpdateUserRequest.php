...
class UpdateUserRequest extends FormRequest
{
    ...
    public function authorize()
    {
        ...
        if ($auth_user->isStudent() && ($auth_user->id !== $userToUpdate->id)) {
            abort(403, 'You are not authorized to update this user.');
        }

        // Prevent student and program chair from updating their own student number
        ...
        // Fetch the user to update and student if needed
        if (!$userToUpdate) {
    ...
    public function prepareForValidation()
    {
        $this->merge([
            ...
            'student' => $this->route('user')->student, // Assuming User has a related student
        ]);
    }
